...
with open('productos_extraccion.csv', mode='w', newline='', encoding='utf-8') as archivo_csv:
    ...
    campos = ['Categoría', 'Página', 'Posición', 'Descripción', 'Precio Normal', 'Precio Descuento', 'Imagen URL', 'Tallas']
    ...
    # Lista de categorías
    categorias_urls = [
        'https://www.offcorss.com/ropa-recien-nacido-nina?page=',
        'https://www.offcorss.com/ropa-recien-nacido-nino?page=',
        'https://www.offcorss.com/ropa-nina?page=',
        'https://www.offcorss.com/ropa-nino?page=',
        'https://www.offcorss.com/ropa-para-adolescentes?page=',
        'https://www.offcorss.com/ropa-juvenil-mujer?page=',
        'https://www.offcorss.com/outlet?page=',
        'https://www.offcorss.com/outlet/ropa-bebe-nina?page='
    ]
    ...
    categorias = {
        ...
        'ropa-nina': 'Niñas',
        'ropa-nino': 'Niños',
        'ropa-para-adolescentes': 'Adolescentes',
        'ropa-juvenil-mujer': 'Adolescentes',
        'outlet': 'Outlet'
    }
    ...
    for categoria in categorias_urls:
        ...
        while True:
            ...
            if not productos:
                logging.info(f"No se encontraron más productos en la página {pagina}. Finalizando extracción para esta categoría.")
                print(f"No se encontraron más productos en la página {pagina}. Finalizando extracción para esta categoría.")
                break
            ...
            for idx, item in enumerate(productos):
                try:
                    # Descripción
                    descripcion = item.select_one('h2').text.strip() if item.select_one('h2') else "Descripción no disponible"
                    ...
                    if len(precios) == 2:
                        precios_float = [float(precio) for precio in precios]
                        precio_normal = max(precios_float)  # El mayor precio es el normal
                        precio_descuento = min(precios_float)  # El menor precio es el descuento
                    elif len(precios) == 1:
                        precio_normal = round(float(precios[0]), 2)  # Solo un precio, es el precio normal
                    ...
                    imagen_element = item.select_one('img.offcorss-apps-newsite-1-x-g-vitrinasShelfImagecontainer__image')
                    imagen_url = (imagen_element['src'] if imagen_element and imagen_element.has_attr('src')
                                  else (imagen_element['data-src'] if imagen_element and imagen_element.has_attr('data-src')
                    ...
                    # Tallas
                    tallas = item.select('div.offcorss-apps-newsite-1-x-g-vitrinasShelf__infoContainer > div.offcorss-apps-newsite-1-x-g-vitrinasShelfInfocontainer__bottom > div > div')
                    ...
                    logging.info(f"Producto {posicion} extraído: {descripcion}, {precio_normal}, {precio_descuento}, {imagen_url}, Tallas: {tallas}")
                    print(f"Producto {posicion} extraído: {descripcion}, {precio_normal}, {precio_descuento}, {imagen_url}, Tallas: {tallas}")
